...
class FeedbackService:
    ...
    def validate_document_exists(self, document_id: int):
        ...
        result = self.collection.get(
            where={"id": str(document_id)}, include=["documents", "metadatas"]
        )
    ...
    def add_to_db(self, input_data: Input, db: Annotated[Session, Depends(get_db)]):
        ...
        if not self.validate_document_exists(input_data.document_id):
            raise HTTPException(
                ...
                detail="Document ID not found in the collection",
            )
        ...
        if existing_feedback:
            ...
            logger.info(
                f"Updated feedback for document {input_data.document_id} by user {input_data.action_by}"
            )
            return {
                "message": "Feedback updated successfully",
                "data": existing_feedback.id,
            }
            ...
            logger.info(
                f"Feedback created for document {input_data.document_id} by user {input_data.action_by}"
            )
