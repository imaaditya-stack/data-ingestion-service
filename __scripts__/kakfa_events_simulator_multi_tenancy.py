...
class ScenarioConfig:
    ...
    tenants: List[str] = field(default_factory=lambda: ["75047507238502352"])
    dry_run: bool = False
...
class RealisticEventSimulator:
    ...
    def __init__(self, config: ScenarioConfig):
        ...
        self._ensure_db_session_manager()
    ...
    async def simulate_entity(self, entity_type: str, csv_path: str, topic: str) -> int:
        ...
        for i, row in enumerate(records, start=1):
            ...
            try:
                ...
                try:
                    event_data = await self._attach_signature(event_data, tenant_id)
                except Exception as sig_error:  # pylint: disable=broad-except
                    logger.error(
                        "Failed to sign event for tenant=%s entity=%s: %s",
                        tenant_id,
                        entity_type,
                        sig_error,
                        exc_info=True,
    ...
    async def _attach_signature(
        self, event_data: Dict[str, Any], tenant_id: str
    ) -> Dict[str, Any]:
        """Add HMAC signature to event payload using tenant secrets."""
        secrets = await self.tenant_secret_manager.get_secrets(tenant_id)
        signature = HMACService.generate_signature(secrets.primary, event_data)
        signed_event = dict(event_data)
        signed_event["signature"] = signature
        return signed_event

    @staticmethod
    def _ensure_db_session_manager() -> None:
        """Make sure the global session manager exists when running standalone scripts."""
        try:
            get_session_manager()
